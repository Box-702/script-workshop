...
- 角色结构化增删改：支持编辑姓名、角色类型、目标、动机、性格、关系、弧光和说话风格。
- 地点结构化增删改：支持编辑地点名称和描述。
- 场景引用维护：场景可切换地点、调整出场角色；删除角色或地点前必须检查是否仍被场景引用。
- 版本保存沿用现有 `script_versions`，所有角色/地点编辑都通过“保存快照”落库。
- 补充前端类型检查，确保结构化编辑不会破坏现有 YAML/JSON 导出。
...
- 后端增加 `get_current_user`，所有项目、版本、run、Agent run、模型 key 查询都按用户隔离。
...
- JSON/YAML diff：按剧名、角色、地点、场景、动作、对白分组展示。
